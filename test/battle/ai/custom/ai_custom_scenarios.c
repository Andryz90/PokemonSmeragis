...
AI_SINGLE_BATTLE_TEST("AI custom scenarios: Shed Tail is preferred in a safe pivot situation")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); Moves(MOVE_SHED_TAIL, MOVE_STRENGTH); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); Moves(MOVE_STRENGTH); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI custom scenarios: Shed Tail is avoided at or below 50 percent HP")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAGS_TRAINER_PROFILE);
        PLAYER(SPECIES_WOBBUFFET) { Speed(50); Moves(MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(100); MaxHP(200); Speed(100); Moves(MOVE_SHED_TAIL, MOVE_STRENGTH); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); Moves(MOVE_STRENGTH); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI custom scenarios: Shed Tail is preferred when faster and threatened with a known KO")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SHED_TAIL) == EFFECT_SHED_TAIL);
        AI_FLAGS(AI_FLAGS_TRAINER_PROFILE | AI_FLAG_OMNISCIENT);
        PLAYER(SPECIES_GENGAR) { Level(100); Speed(50); Moves(MOVE_NIGHT_SHADE); }
        OPPONENT(SPECIES_WOBBUFFET) { MaxHP(100); HP(100); Speed(100); Moves(MOVE_SHED_TAIL, MOVE_STRENGTH); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); Moves(MOVE_STRENGTH); }
    } WHEN {
        TURN { MOVE(player, MOVE_NIGHT_SHADE); EXPECT_MOVE(opponent, MOVE_SHED_TAIL); EXPECT_SEND_OUT(opponent, 1); }
    }
}
